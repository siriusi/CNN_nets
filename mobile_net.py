...
class MobileNet:
    ...
    def inference(self, scope='squeeze_net'):  # inference squeeze net
        with tf.variable_scope(scope):
            net = self._conv2d(self.resized_data, 32, [3,3], stride=1, scope="conv_1")
            
            net = self._separable_conv2d(net, 64, [3,3], stride=1, scope="dw_conv_2")
            net = self._separable_conv2d(net, 128, [3,3], stride=1, scope="dw_conv_3")
            net = self._separable_conv2d(net, 128, [3,3], stride=1, scope="dw_conv_4")
            net = self._separable_conv2d(net, 256, [3,3], stride=1, scope="dw_conv_5")
            net = self._separable_conv2d(net, 256, [3,3], stride=1, scope="dw_conv_6")
            net = self._separable_conv2d(net, 512, [3,3], stride=2, scope="dw_conv_7") #输出：16 * 16 * 512
            
            for i in range(5):
                net = self._separable_conv2d(net, 512, [3,3], stride=1, scope="dw_conv_" + str(8 + i))
            
            net = self._separable_conv2d(net, 1024, [3,3], stride=2, scope="dw_conv_13")
            net = self._separable_conv2d(net, 1024, [3,3], stride=1, scope="dw_conv_14") #输出：8 * 8 * 1024
    ...
    def _separable_conv2d(self, input_tensor, num_outputs, kernel_size, stride=1, scope=None):
        return layers.separable_conv2d(input_tensor, num_outputs, kernel_size, depth_multiplier = 1, stride=stride, \
                      scope=scope,normalizer_fn=layers.batch_norm, \
                      normalizer_params={'is_training': self.is_training, "fused" : True, "decay" : self.normalize_decay})
    
    def _conv2d(self, input_tensor, num_outputs, kernel_size, stride=1, scope=None):
